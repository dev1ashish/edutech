 ...
 * Main dropdown = base lists only.
 * B.Tech / M.Tech CSE–related choices unlock a specialization step in the UI.
 */

export type UUProgramLevel = "UG" | "PG";

/** Programmes that show the specialization checkbox once selected (Computer Science Engineering line). */
...
/** Prospectus-aligned CSE honours / track lines (shown when specialization is enabled for UG CSE). */
...
/** PG CSE intake focus options (paired with parent M.Tech CSE listing). */
...
/** Base-only programme list for the level (no merged specialization rows). */
export function getUUBaseProgramOptions(level: UUProgramLevel | ""): string[] {
  if (!level) return [];
  const base = level === "UG" ? UG_BASE : PG_BASE;
  const core = sortedUnique([...base]);
  return [...core.filter((p) => p !== OTHER), OTHER];
}
